/* 
     ...
     live
*/
package org.opendelos.vodapp.api.scheduler;
...
import org.opendelos.vodapp.services.scheduler.LiveService;
...
public class TimeTableApi {
	...
	@Autowired
	public TimeTableApi(ScheduleService scheduleService, DepartmentService departmentService, SchoolService schoolService, OpUserService opUserService, CourseService courseService, LiveService liveService) {
		...
		if (scheduleQuery.getType() != null) {
			if (scheduleQuery.getType().equals("lecture")) {
				...
				if (timetable_event_search_filters != null) {
					request.getSession()
							.setAttribute("timetable_events_search_history", timetable_event_search_filters);
				}
			}
		}
		else {
		...
		for (ScheduleDTO scheduleDTO : scheduleDTOList) {
			...
			if (scheduleDTO.getType().equals("lecture")) {
				String scheduledEventTitle = scheduleDTO.getCourse().getTitle() +
						" - Τμήμα " + scheduleDTO.getDepartment().getTitle() + " - " + scheduleDTO.getSupervisor()
						.getName();
			...
			LocalDateTime endDateTime = startDateTime.plus(lDurationHours, ChronoUnit.HOURS)
					.plus(lDurationMinutes, ChronoUnit.MINUTES);
	...
	public List<ScheduleDTO> getNextBroadcastsExcludingLive() {
		...
		List<ScheduleDTO> passed_list = new ArrayList<>();
		for (ScheduleDTO scheduleDTO : nextBroadcastList) {
			LocalDate date = LocalDate.parse(scheduleDTO.getDate());
			LocalTime startTime = LocalTime.parse(scheduleDTO.getStartTime());
			LocalDateTime startDateTime = date.atTime(startTime);
			long lDurationHours = scheduleDTO.getDurationHours();
			...
			if (startDateTime.isBefore(now) && endDateTime.isAfter(now)) {
				passed_list.add(scheduleDTO);
			}
			if (endDateTime.isBefore(now)) {
				passed_list.add(scheduleDTO);
			}
			if (startDateTime.isAfter(now)) {
				scheduleDTO.setBroadcast_time("future");
			}
		}
		if (!passed_list.isEmpty()) {
			nextBroadcastList.removeAll(passed_list);
		}
	...
	public QueryResourceResults getLiveHomeScheduledEvent(HttpServletRequest request,
		...
		return liveService.getLiveResourcesByQuery(resourceQuery);
	}
}
