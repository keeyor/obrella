            ...
            success: function(data){
                data.data.forEach(function(item) {
                    ...
                    if (item.access === "public") {
                      _html += '<td style="width: 92%" class="pl-0"><a href="daily?' + queryParams + '">Δημόσια</a></td>';
                    }
                    else if (item.access === "private") {
                      _html += '<td style="width: 92%" class="pl-0"><a href="daily?' + queryParams + '">Ιδιωτική</a></td>';
                    }
                    else if (item.access === "open") {
                        _html += '<td style="width: 92%" class="pl-0"><a href="daily?' + queryParams + '">Ανοικτή</a></td>';
                    }
                    else if (item.access === "sso") {
                        _html += '<td style="width: 92%" class="pl-0"><a href="daily?' + queryParams + '">Ιδρυματικός Λογαριασμός</a></td>';
                    }
                    else if (item.access === "password") {
                        _html += '<td style="width: 92%" class="pl-0"><a href="daily?' + queryParams + '">Κωδικός Πρόσβασης</a></td>';
                    }
