...
function capture(event) {
	...
	if (event.type === "play") {
		...
		curr_time += 1;
		if (debug) info.innerHTML = "onSeek" + curr_time;
		for (let i = 0; i < cuepoints_c.length; i++) {
			if (cuepoints_c[i] <= curr_time) {
				...
				if (diff <= 0) {
					if (debug) info.innerHTML = "on Seek of end-cut current time at " + curr_time + " cut_period " + i + ":" + cuepoints_c[i] + " - " + end_c;
					if ((end_c / 1000) < global_vid_duration) {
						getVideo().currentTime = end_c / 1000.0; //$f().seek(end_c / 1000.0);
					} else {
						getVideo().play(); //$f().play();
					}
					break;
				} else {
					if (debug) info.innerHTML = "no cuts:" + i + " " + cuepoints_c[i] + " " + end_c + " |curr: " + curr_time + " dif: " + diff;
				}
			}
		}
		...
		getVideo().textTracks.addEventListener('addtrack', function () {
			...
			tt.addEventListener('cuechange', function () {
				if (tt.activeCues[0] !== undefined) {
					if (debug) {info.innerHTML = " Seeked to:" + tt.activeCues[0].endTime + "<br/><br/>";}
					getVideo().currentTime = tt.activeCues[0].endTime;
				} else {
					if (debug) {info.innerHTML = "Cue point duration over" + "<br/><br/>";}
				}
			}); //end oncuechange
		});
		getVideo().volume = 0.5;
