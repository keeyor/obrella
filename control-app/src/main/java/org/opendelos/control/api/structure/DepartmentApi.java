...
public class DepartmentApi {
	...
	@RequestMapping(value= "/api/v2/s2/departments-only.web/authorized/{access}", method = RequestMethod.GET, produces =  "application/json")
	public ResponseEntity<String> getAuthorizedDepartments(@PathVariable String access) {
		...
		try {
			groupChildren.sort(Comparator.comparing(Select2GenChild::getText));
			String s2departments = ApiUtils.FormatResultsForSelect2(groupChildren);
			return new ResponseEntity<>(s2departments, HttpStatus.ACCEPTED);
		}
		catch (Exception e) {
			return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
