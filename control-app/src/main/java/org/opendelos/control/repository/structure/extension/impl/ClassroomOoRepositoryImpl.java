...
public class ClassroomOoRepositoryImpl implements ClassroomOoRepository {
	...
	public List<Classroom> n_findAssignedClassrooms(List<String> departmentIds, String usage) {
		...
		if (usage != null) {
		 ...
		 //!! remove paragraph to allow scheduling to all Classrooms (and not only assigned to department)
		 if (usage != null && usage.equals("lectures")) {
			 //# Get only classrooms available to DepartmentIds
			 expression.and("availableTo").in(departmentIds);
			 andExpression.add(expression);
		 }
