/* 
     ...
     live
*/
package org.opendelos.control.repository.structure.extension.impl;
...
public class CourseOoRepositoryImpl implements CourseOoRepository {
	...
	public List<Course> findAllAssignedByDepartmentId(String departmentId) {
		...
		expression.orOperator(
				Criteria.where("department._id").is(objID),
				Criteria.where("teachingCounter").gt(0)
		);
	...
	@Override
	public List<Course> findAllAssignedCourses() {
		Query query = new Query(Criteria.where("teachingCounter").gt(0));
		Sort sort = Sort.by(Sort.Order.asc("title"));
		query.with(sort);
		return mongoTemplate.find(query, Course.class);
	}
	...
	public void findAndUpdate(Course course) {
		...
		update.set("categories", course.getCategories());
		...
		update.set("departmentsRelated", course.getDepartmentsRelated());
