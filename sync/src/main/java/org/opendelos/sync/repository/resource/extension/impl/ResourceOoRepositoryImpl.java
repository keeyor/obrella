/* 
     ...
     live
*/
package org.opendelos.sync.repository.resource.extension.impl;
...
public class ResourceOoRepositoryImpl implements ResourceOoRepository {
	...
	@Override
	public Resource findByIdentityInCollection(String identity, String collectionName) {
		Criteria expression = new Criteria();
		expression.and("identity").is(identity);
		Query query = new Query();
		query.addCriteria(expression);
	...
	private Criteria setSupportSecurityRestrictions(ResourceQuery resourceQuery) {
		...
		for (UserAccess.UserRights.CoursePermission coursePermission : resourceQuery.getAuthorized_courses()) {
			...
			course_expression.andOperator(Criteria.where("course._id").is(new ObjectId(coursePermission.getCourseId())),
					Criteria.where("supervisor._id").is(new ObjectId(coursePermission.getStaffMemberId())));
