        ...
        if ( departmentFilterId === '') {
            ...
            if (department_title != null && department_title !== "") {
                if (exists(departments_report, calEvent.event._def.extendedProps.departmentId) === false) {
                    ...
                    //tmp
                    let depDtId = calEvent.event._def.extendedProps.departmentId;
                    let queryParams = new URLSearchParams(window.location.search);
                    queryParams.set("d", depDtId);
                    queryParams.delete("skip");
                    let html = '<li class="list-group-item">' +
                        '<a class="text-dark text-decoration-none" href="calendar?' + queryParams + '">' + department_title + '</a>' +
                        '</li>';
...
/*        let classroomFilterId = $("#classRoomFilterId").val()
        ...
        if ( courseFilterId === '') {
            if (calEvent.event._def.extendedProps.type === "lecture") {
                if (exists(courses_report, calEvent.event._def.extendedProps.resourceId) === false) {
                    ...
                    let courseDtId = calEvent.event._def.extendedProps.resourceId;
                    let queryParams = new URLSearchParams(window.location.search);
                    queryParams.set("c", courseDtId);
                    queryParams.delete("skip");
                    let html = '<li class="list-group-item">' +
                        '<a class="text-dark text-decoration-none" href="calendar?' + queryParams + '">' + res_title + '</a>' +
                        '<div class="font-sm text-muted"> Τμήμα ' + calEvent.event._def.extendedProps.department + '</div>' +
                        '</li>';
        ...
        if (staffMemberFilterId === '') {
            if (exists(staff_report, calEvent.event._def.extendedProps.supervisorId) === false) {
                    ...
                    let staffDtId = calEvent.event._def.extendedProps.supervisorId;
                    let queryParams = new URLSearchParams(window.location.search);
                    queryParams.set("s", staffDtId);
                    queryParams.delete("skip");
                    let html = '<li class="list-group-item">' +
                        '<a class="text-dark text-decoration-none" href="calendar?' + queryParams + '">' + calEvent.event._def.extendedProps.supervisor + '</a>' +
                        '<div class="font-sm text-muted"> Τμήμα ' + calEvent.event._def.extendedProps.department + '</div>' +
                        '</li>';
                    $("#stFilters").append(html);
            }
        }
        if (courseFilterId !== "" && departmentFilterId !== "" && staffMemberFilterId !== "") {
            $("#no_dyna_filters").html("δεν βρέθηκαν επιπλέον φίλτρα.");
        }
        if (courseFilterId === "" && departmentFilterId === "" && staffMemberFilterId === "") {
            $("#applied_filters").html("κανένα επιλεγμένο φίλτρο");
        }
    }
