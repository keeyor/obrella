/* 
     ...
     live
*/
package org.opendelos.liveapp.repository.resource.extension.impl;
...
public class ResourceOoRepositoryImpl implements ResourceOoRepository {
	...
	public List<Resource> findRelatedPartsById(String id, String accessPolicy) {
		...
		Criteria expression = new Criteria();
		ObjectId objID = new ObjectId(id);
		expression.orOperator(Criteria.where("_id").is(objID), Criteria.where("parentId").is(id));
		andExpression.add(expression);
	...
	private List<Criteria> setQueryCriteria(ResourceQuery resourceQuery) {
		...
		if (resourceQuery.getCategoryCode()!= null && !resourceQuery.getCategoryCode().isEmpty()) {
			Criteria expression = new Criteria();
			expression.orOperator(Criteria.where("categories").regex(resourceQuery.getCategoryCode()),
					Criteria.where("course.categories").regex(resourceQuery.getCategoryCode()));
			andExpression.add(expression);
		}
