/* 
     ...
     obrella
*/
package org.opendelos.liveapp.services;
...
public class LiveRunner {
	...
	private final Logger logger = LoggerFactory.getLogger(LiveRunner.class);
...
/*
	...
	public void ScheduleRunner() {
		...
		if (TodaySchedule.size()>0) {
			logger.info("SCHEDULER RUN AT '{}'", localDateTime);
...
/* STREAM_STATUS RETURNS:
				...
				if (streams_stop_now >0 || streams_start_now >0 || streams_restart >0 || streams_passed >0 || streams_live >0 || streams_future > 0 || streams_errors >0) {
					logger.info("SCHEDULER REPORT AT '{}'. Stopped: '{}'. Started: '{}'. ReStarted: '{}'. Passed: '{}'. Live Now: '{}'. Future Live: '{}'. Errors: '{}'",
				...
				logger.info("SCHEDULER REPORT AT '{}'. No Streaming Servers configured or none enabled. Nothing to do...", localDateTime);
			}
		}
		else {
			logger.info("SCHEDULER REPORT AT '{}'. No Live Streams for today!", localDateTime);
		}
	}
*/
