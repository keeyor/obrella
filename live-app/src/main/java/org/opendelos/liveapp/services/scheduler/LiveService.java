...
import org.opendelos.liveapp.conf.ChannelProperties;
...
/*                        //Get StreamingServer with Min Usage and Assign to NEW ITEM
    ...
    public void AddScheduleDTOToTodaysSchedule(ScheduleDTO scheduleDTO) {
        ...
        if (!ssMap.isEmpty()) {
            ...
            logger.info("No Streaming Servers configured or Enabled!");
        }
    }
    ...
    public  StreamingServer getRecorderServer() {
        ...
        List<StreamingServer> streamingServers = streamingServerService.getAllByEnabledAndType("true","recorder");
        if (streamingServers != null && streamingServers.size() > 0) {
            for (StreamingServer streamingServer: streamingServers) {
    ...
    public Long getScheduledForTodayCoursesCount() {
        ...
        queryResourceResults = resourceService.searchPageableLectures(resourceQuery);
        if (queryResourceResults != null && queryResourceResults.getTotalResults() >0) {
            liveCounter =  queryResourceResults.getTotalResults();
        }
        return liveCounter;
    }
    ...
    public int getLiveLecturesCounter() throws ExecutionException, InterruptedException, UnsupportedEncodingException {

        int liveCourseCounter = 0;
        ResourceQuery resourceQuery = new ResourceQuery();
        resourceQuery.setCollectionName("Scheduler.Live");
        resourceQuery.setSort("date");
        resourceQuery.setDirection("asc");
        ...
        //> Get Live List
        QueryResourceResults liveResources = this.getLiveResourcesByQuery(resourceQuery);
        if (liveResources != null && !liveResources.getSearchResultList().isEmpty()) {
            liveCourseCounter = liveResources.getSearchResultList().size();
        }
        return liveCourseCounter;
    }
    ...
    public Resource createLiveChannelResourceFromScheduleDTO(ScheduleDTO scheduleDTO) {
        ...
        Resource liveEntry = new Resource();
        //STREAMID :: CHANNEL SPECIFIC -> look at channel.properties
...
/*        String streamId;
    ...
    public Map<Integer,String> getStreamingServersMaps() {
        ...
        List<StreamingServer> streamingServers = streamingServerService.getAllIdsByEnabledAndType("true","ipcamera");
        if (streamingServers != null && streamingServers.size() > 0) {
            int index = 0;
            for (StreamingServer streamingServer: streamingServers) {
    ...
    public String getEncryptedStreamName(String str, String algorithm)   {

        StringBuilder sb = new StringBuilder();
        try {
            MessageDigest md = MessageDigest.getInstance(algorithm); //.getInstance("SHA-1") || .getInstance(MD5")
