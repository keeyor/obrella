/* 
     ...
     live
*/
package org.opendelos.live.repository.extension.impl;
...
public class ResourceOoRepositoryImpl implements ResourceOoRepository {
	...
	public QueryResourceResults searchPageableLectures(ResourceQuery resourceQuery) {
		...
		if (resourceQuery.getAccessPolicy() !=null && resourceQuery.getAccessPolicy().equals("private")) {
			Criteria expression = new Criteria();
			expression.and("accessPolicy").is("private");
			andExpression.add(expression);
		}
		else  if (resourceQuery.getAccessPolicy() !=null && resourceQuery.getAccessPolicy().equals("public")) {
			Criteria expression = new Criteria();
		...
		else if (resourceQuery.isManager() && resourceQuery.getAuthorizedUnitIds().size() > 0 && !resourceQuery.getAuthorizedUnitIds().contains("_all")){
			// If user is Manager of Units -> return resources.courses where department (of course) in authorized list
			// and resources.event where responsibleUnit is in authorized list (no matter what and if responsible person exists) -> be careful : this needs tio include schools, institution in authorizedDepartmentIds
			Criteria expression = new Criteria();
			expression.orOperator(Criteria.where("course.department._id").in(resourceQuery.getAuthorizedUnitIds()),
					Criteria.where("event.responsibleUnit._id").in(resourceQuery.getAuthorizedUnitIds()));
			andExpression.add(expression);
		}
		...
		if (resourceQuery.getCategoryCode()!= null && !resourceQuery.getCategoryCode().isEmpty()) {
			Criteria expression = new Criteria();
			expression.and("categories").regex(resourceQuery.getCategoryCode());
			andExpression.add(expression);
		}
		...
		if (resourceQuery.getDate()!= null && !resourceQuery.getDate().isEmpty()) {
			Criteria expression = new Criteria();
			//trying to get lectures within lectureQuery.getDate()
	...
	public List<Resource> searchLecturesOnFilters(ResourceQuery resourceQuery) {
		...
		if (resourceQuery.getDepartmentId() != null && !resourceQuery.getDepartmentId().isEmpty()) {
			ObjectId objID = new ObjectId(resourceQuery.getDepartmentId());
			Criteria expression = new Criteria();
			expression.orOperator(Criteria.where("department._id").is(objID),
			Criteria.where("event.responsibleUnit._id").is(objID));
			andExpression.add(expression);
		}
		if (resourceQuery.getStaffMemberId() != null && !resourceQuery.getStaffMemberId().isEmpty()) {
	...
	@Override
	public void deleteFromCollectionById(String id, String collectionName) {
		Resource resource = mongoTemplate.findById(id,Resource.class,collectionName);
		if (resource != null) {
			mongoTemplate.remove(resource, collectionName);
		}
	}
