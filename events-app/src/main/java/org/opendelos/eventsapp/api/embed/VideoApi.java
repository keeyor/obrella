/* 
     ...
     live
*/
package org.opendelos.eventsapp.api.embed;
...
public class VideoApi {
	...
	public ResponseEntity<EmdedResponse> getUrlForResourceId(@PathVariable("id") String id, Locale locale) {
		...
		if (resource != null && resource.getResourceAccess() != null && resource.getAccessPolicy().equals("public")) {
			url_builder.append(streaming_protocol).append("://").append(streaming_host).append(":").append(streaming_port).append("/").append(streaming_webdir);
			...
			if (resourceAccess.getFolder() != null && resourceAccess.getFileName() != null) {
				...
				if (resource.getType().equals("COURSE")) {
					emdedResponse.setSupervisor(resource.getSupervisor().getName() + ", " + resource.getSupervisor().getAffiliation());
				}
				StringBuilder subtitle_builder = new StringBuilder();
				if (resource.getDepartment() != null) {
					subtitle_builder.append("Τμήμα ").append(resource.getDepartment().getTitle()).append(", ").append(defaultInstitution.getTitle());
				}
				else {
			...
			url_builder.append(streaming_protocol).append("://").append(streaming_host).append(":").append(streaming_port).append(support_files_webDir);
