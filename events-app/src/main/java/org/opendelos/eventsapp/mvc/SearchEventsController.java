...
public class SearchEventsController {
	...
	@Autowired
	public SearchEventsController(OptionServices optionServices, ResourceService resourceService, ResourceUtils resourceUtils, DepartmentService departmentService, MultilingualServices multilingualServices) {
	...
	public String getHomePage(final Model model,HttpServletRequest request, Locale locale,
		...
		resourceQuery.setEventArea("ea_es");
		...
		prepareLinkReplacements(model,urlString);
			...
			List<Select2GenGroup> departmentsBySchool = departmentService.getAllDepartmentsGroupedBySchool("", locale);
			model.addAttribute("unitFilter", departmentsBySchool);

			//Areas
		    model.addAttribute("areaTitle", multilingualServices.getValue("ea_es",null,locale));
			String[] subAreasList  = optionServices.getSubAreasOfAreaByKey("ea_es",locale);
	...
	@RequestMapping(value = {"/search-evs"}, method = RequestMethod.POST)
