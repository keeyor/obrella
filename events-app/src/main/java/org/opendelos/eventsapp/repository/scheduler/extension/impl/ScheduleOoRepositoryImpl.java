/* 
     ...
     live
*/
package org.opendelos.eventsapp.repository.scheduler.extension.impl;

import java.time.LocalDate;
import java.util.ArrayList;
import java.util.List;

import org.opendelos.eventsapp.repository.scheduler.extension.ScheduleOoRepository;
...
public class ScheduleOoRepositoryImpl implements ScheduleOoRepository {
	...
	public List<Schedule> search(ScheduleQuery scheduleQuery) {
		...
		if (scheduleQuery.getType() !=null && !scheduleQuery.getType().equals("_all")) {
			...
			if (scheduleQuery.getStatus_type() != null && !scheduleQuery.getStatus_type().equals("_all")) {
				if (scheduleQuery.getStatus_type().equals("past")) {
					LocalDate today = LocalDate.now();
					//System.out.println(("_past filter. compare to:" + today));
					expression = new Criteria();
					expression.and("date").lt(today);
					andExpression.add(expression);
				}
				else if (scheduleQuery.getStatus_type().equals("future")) {
					LocalDate today = LocalDate.now();
					//System.out.println(("_future filter. compare to:" + today));
					expression = new Criteria();
					expression.and("date").gt(today);
					andExpression.add(expression);
				}
				else if (scheduleQuery.getStatus_type().equals("today")) {
					LocalDate today = LocalDate.now();
					//System.out.println(("_today filter. compare to:" + today));
					expression = new Criteria();
					expression.and("date").is(today);
					andExpression.add(expression);
				}
			}
		}
...
/*		if (scheduleQuery.getType() != null && !scheduleQuery.getType().equals("_all")) {
			Criteria expression = new Criteria();
			List<Criteria> andCourseExpression =  new ArrayList<>();
			if (scheduleQuery.getType().equals("events")) {
				andCourseExpression.add(Criteria.where("event").exists(true));
			}
			else  {
				andCourseExpression.add(Criteria.where("course").exists(true));
			}
			expression.andOperator(new Criteria().andOperator(andCourseExpression.toArray(new Criteria[andCourseExpression.size()])));
			andExpression.add(expression);
		}*/
		//Security Restrictions
		if (scheduleQuery.isManager() && !scheduleQuery.isSA()) {
...
/*		if (scheduleQuery.getType() !=null && !scheduleQuery.getType().equals("_all")) {
		...
		if (scheduleQuery.getEvent_type() != null && !scheduleQuery.getEvent_type().equals("_all")) {
			Criteria expression = new Criteria();
			expression.and("date").is(scheduleQuery.getDate().atStartOfDay());
			andExpression.add(expression);
		}
...
//Sort.by(Sort.Order.asc(scheduleQuery.getSortBy())).and(Sort.Order.asc( "startTime"));
	...
	private Criteria setManagerSecurityRestrictions(ScheduleQuery scheduleQuery) {
		Criteria expression = new Criteria();
		if (scheduleQuery.getAuthorizedUnitIds() != null  && !scheduleQuery.getAuthorizedUnitIds().contains("IGNORE_UNIT")) {
			List<Criteria> orCourseExpression =  new ArrayList<>();
