(function () {
    ...
    function RestoreCheckedItemsFromFilters() {
        ...
        let terms_html='';
        let search_terms = $("#search-box").val();
        if (search_terms !== "") {
            $("#filters-header").show();
            $('.remove_all_filters').show();
            terms_html = "<b><u>Αναζήτηση κειμένου:</u></b> " + search_terms +
                '<a  title="' + lang.u1 + '" href="#" class="remove_terms_filter mx-2" data-target="search-box">' +
                '<i style="color: red" class="fas fa-minus-circle"></i></a>';
            let $termsFilters = $("#termsFilters");
            $termsFilters.html(terms_html);
        }
            ...
            $checkbox_elements.each(function () {
                ...
                if (departmentFilterId === element_id) {
                    ...
                    filters_dep_html += $('#' + element_id + '_text').text() +
                        '<a  role="button" class="btn btn-sm btn-secondary" title="' + lang.u1 + '" href="#" class="remove_dep_filter mx-2" data-target="' + element_id + '">' +
        ...
        let eventAreaFilterId = $("#eventAreaFilterId").val();
        let filters_area_html='';
        if (eventAreaFilterId !== "") {
            let $checkbox_elements = $('.event-area-filter');
            $checkbox_elements.each(function () {
                let element_id = $(this).attr("id");
                if (eventAreaFilterId === element_id) {
                    $(this).prop("checked", true);
                    filters_area_html += $('#' + element_id + '_text').text() +
                        '<a  title="' + lang.u1 + '" href="#" class="remove_area_filter mx-2" data-target="' + element_id + '">' +
                        '<i style="color: red" class="fas fa-minus-circle"></i></a>';
                }
            });
            if ($checkbox_elements.length > 0) {
                $("#filters-header").show();
                $('.remove_all_filters').show();
                filters_area_html = "<b><u>" + lang.u7 + ":</u></b> " + filters_area_html;
                let $eventAreaTextFilters = $("#eventAreaTextFilters");
                $eventAreaTextFilters.html(filters_area_html);
    ...
    function onTermsFilterRemove() {
        $(".remove_terms_filter").on('click',function (){
            $("#search-box").val('');
            $("#search-form").submit();
        })
    }

    function onAreaFilterRemove() {
        $(".remove_area_filter").on('click',function (){
            let id = $(this).data("target");
            let el = $('#' + id);
            el.prop("checked", false);
            triggerEventAreaFilterChange(el);
        })
    }
    ...
    function onEventAreaFilterChange() {
        $(".event-area-filter").on('change', function(){
            let el = $(this);
            triggerEventAreaFilterChange(el);
        });
    }
    ...
    function triggerEventAreaFilterChange(el) {
        let $checkbox_elements = $('.event-area-filter');
        let checked_id = el.attr("id");
        $checkbox_elements.each(function() {
            let elem_id = $(this).attr("id");
            if (elem_id !== checked_id) {
                $(this).prop("checked",false);
            }
        });
        //check if element that was checked -> unchecked
        if (el.prop("checked")) {
            $("#eventAreaFilterId").val(checked_id);
        }
        else {
            $("#eventAreaFilterId").val("");
        }
